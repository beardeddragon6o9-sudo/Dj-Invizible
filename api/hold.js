...
# Write a clean version (no requestBody.id)
@'
...
// Keep a tracking id for your hold, but DO NOT use it as the event's id
function holdIdFrom({ start, end, email }) {
  const raw = `${start}|${end}|${email || ""}`;
  return "hold" + Buffer.from(raw).toString("hex").slice(0, 40); // lowercase hex only (safe as a value, but we won't set as event.id)
}
...
async function createHold(cal, calendarId, payload) {
  ...
  const requestBody = {
    // IMPORTANT: no `id` here â€” let Google generate it
    summary,
    ...
    extendedProperties: {
      private: {
        hold: "true",
        holdId, // your tracking id lives here only
  ...
  return {
    id: event.id, // Google's event id (valid format)
    calendarId,
...
module.exports = async (req, res) => {
  try {
    ...
    if (req.method === "POST") {
      ...
      const hold = await createHold(cal, calendarId, req.body);
      return ok(res, { hold });
    }
...
git add api\hold.js
git commit -m "fix(hold): remove event id from requestBody; keep holdId in private props"
git push
